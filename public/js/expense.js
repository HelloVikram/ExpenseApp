const endpoint=`http://localhost:3000`
      ...
      async function getexpense(page = 1) {
        try {
          ...
          updatePaginationControls(response.data.totalPages,response.data.currentPage);
        } catch (err) {
       ...
       console.log(obj);
        try {
          const response = await axios.delete(
            `${endpoint}/expense/delete-expense/${obj._id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        try {
          ...
          Button.addEventListener("click", (e) => {
            ...
            response.data.forEach((el) => {
              const li = document.createElement("li");
              li.textContent = `Name:${el.username}-TotalExpense:${el.totalexpenses}`;
